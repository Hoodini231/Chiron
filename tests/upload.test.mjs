...
function harness({ throws = [0.2], duration = 3, saveFails = false } = {}) {
  ...
  class Element extends EventTarget {
    ...
    querySelectorAll() {
      return this.children.flatMap((c) =>
        typeof c === 'object' ? [c, ...c.querySelectorAll()] : [],
      );
    }
    pause() {
      this.paused = true;
  ...
  class Recorder {
    ...
    stop() {
      this.state = 'inactive';
      this.ondataavailable({ data: new Blob(['processed']) });
      this.onstop();
    }
  }
  const context = {
    ...
    window: { MediaRecorder: Recorder, addEventListener() {} },
      ...
      return {
        ...
        json: async () =>
          saveFails
            ? { error: 'disk full' }
            : url === '/api/results'
              ? { id: 'test', processed: 'processed.webm', original: 'original.webm' }
              : {},
      };
    },
    drawPose() {},
    ...
    processThrows,
    seekVideo,
    checkEncoderSupport: async () => {},
    createVideoWriter: async () => ({
      add: async (t, d) => encoded.push([t, d]),
      finish: async () => new Blob(['processed'], { type: 'video/webm' }),
      cancel: async () => {},
    }),
    poseAt: () => throwSample($('source').currentTime, throws).pose,
  };
...
test('uploaded video scans, reviews, then saves only retained samples and unchanged original', async () => {
  const { api, $, requests, encoded } = harness();
  const file = new File(['original bytes'], 'throw.webm', { type: 'video/webm' });
  await api.uploadVideo(file);
  assert.equal(api.state().uploadedFile, file);
  assert.equal($('start').disabled, false);
  await api.startRecording();
  assert.equal(api.state().recording, null);
  assert.equal(api.state().throwWindows.length, 1);
  assert.equal(requests.length, 0);
  const row = $('throw-windows').children[0];
  const inputs = row.children
    .filter((c) => c.tagName === 'LABEL')
    .map((label) => label.children[1]);
  inputs[0].value = '0.3';
  inputs[0].oninput();
  inputs[1].value = '1.8';
  inputs[1].oninput();
  ...
  assert.equal(report.samples[0].t_s, 0);
...
test('zero detections allow a manual window and invalid edits disable processing', async () => {
  ...
  await api.uploadVideo(new File(['x'], 'empty.webm', { type: 'video/webm' }));
  await api.startRecording();
  assert.equal(api.state().throwWindows.length, 0);
  assert.equal($('process-throws').disabled, true);
  assert.match($('throw-review-message').textContent, /No throws/);
  $('source').currentTime = 0.3;
  $('add-throw').onclick();
  assert.equal($('process-throws').disabled, false);
  const row = $('throw-windows').children[0],
    start = row.children.find((c) => c.tagName === 'LABEL').children[1];
  start.value = '';
  start.oninput();
  assert.equal($('process-throws').disabled, true);
  start.value = '.4';
...
test('save failure keeps finished export for retry and disables further processing', async () => {
  ...
  await api.uploadVideo(new File(['x'], 'throw.webm', { type: 'video/webm' }));
  await api.startRecording();
  await api.processApprovedThrows();
  assert.ok(api.state().pendingSave);
  assert.equal($('process-throws').disabled, true);
  assert.equal($('retry-save').hidden, false);
});
test('camera recording remains untrimmed and uses the existing recorder save transaction', async () => {
  const { api, requests } = harness();
  api.setCamera();
  await api.startRecording();
  assert.ok(api.state().recording.raw);
  api.frame(performance.now(), { mediaTime: 10 });
  api.frame(performance.now() + 50, { mediaTime: 10.05 });
  await api.stopRecording();
  assert.equal(requests.length, 4);
  const report = JSON.parse(requests[0].body).report;
  assert.equal(report.capture.source, 'camera');
  assert.equal(report.throw_detection, undefined);
  assert.equal(report.samples.length, 2);
  assert.equal(report.samples[0].t_s, 0);
});
test('unsupported and oversized uploads are rejected before replacing a source', async () => {
  const { api, $ } = harness();
  await api.uploadVideo(new File(['x'], 'bad.txt', { type: 'text/plain' }));
  assert.equal(api.state().uploadedFile, null);
  assert.match($('error').textContent, /MP4 or WebM/);
  await api.uploadVideo({ size: 161 * 1024 * 1024 });
  assert.match($('error').textContent, /160 MB/);
  $('source').duration = 121;
  await api.uploadVideo(new File(['x'], 'long.mp4', { type: 'video/mp4' }));
  assert.equal(api.state().uploadedFile, null);
  assert.match($('error').textContent, /2 minutes/);
});
