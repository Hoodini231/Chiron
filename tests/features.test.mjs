...
test('angles use image dimensions and suppress occluded joints',()=>{
  ...
  assert.equal(jointAngle(point(0,0,.1),point(0,1),point(1,1),640,360),null);
...
test('missing detections remain missing and unavailable physics stay null',()=>{
  const result=summarize({capture:{duration_s:1},samples:[{ball:{},pose:[]},{ball:null,pose:Array(33).fill({})}]});
  assert.equal(result.ball_detection_fraction,.5);
  assert.equal(result.pose_detection_fraction,.5);
