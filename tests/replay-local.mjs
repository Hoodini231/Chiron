...
const dir='/tmp/dodgeball-frames', times=JSON.parse(fs.readFileSync(dir+'/times.json'));
const report=JSON.parse(fs.readFileSync('results/52cc1e26f37d44459bf27ed0ae7fb4b3/data.json'));
const tracker=new BallTracker(cv), mat=new cv.Mat(360,640,cv.CV_8UC4);cv.imread=()=>mat.clone();
const output=[];let j=0;
for(let i=0;i<times.length;i++){
 const t=times[i];while(j+1<report.samples.length && report.samples[j+1].t_s<t)j++;
 mat.data.set(fs.readFileSync(dir+'/'+String(i).padStart(4,'0')+'.rgba'));
 const sample=report.samples[j];output.push({t,ball:tracker.detect({},t,report.tracking,{hands:sample.hands,pose:sample.pose})});
}
fs.writeFileSync(process.argv[2],JSON.stringify(output));mat.delete();
