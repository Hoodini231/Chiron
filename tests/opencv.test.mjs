...
test('strong colour core separates a red ball touching a skin-coloured patch', () => {
  ...
  try {
    cv.rectangle(
      mat,
      new cv.Point(80, 90),
      new cv.Point(150, 115),
      new cv.Scalar(200, 105, 70, 255),
      -1,
    );
    ...
    assert.ok(ball);
    assert.equal(ball.mask_source, 'core');
...
test('clear circular edges can acquire a less saturated ball and maintain its track', () => {
  ...
  try {
    ...
    const ball = tracker.detect({}, 0.05, config);
    assert.ok(ball);
    assert.ok(Math.abs(ball.x - 85) < 2);
  } finally {
    cv.imread = original;
    mat.delete();
  }
...
test('actual OpenCV finds all four synthetic coloured balls and rejects blank frames', () => {
  ...
  try {
    for (const [name, colour] of Object.entries(colours)) {
      ...
      cv.imread = () => mat.clone();
      const tracker = new BallTracker(cv),
        ball = tracker.detect({}, 0, { hue: PRESETS[name], tolerance: 14, saturation: 85 });
      assert.ok(ball, name + ' detection');
      assert.ok(Math.hypot(ball.x - 120, ball.y - 90) < 1);
      mat.setTo(new cv.Scalar(10, 10, 10, 255));
      assert.equal(
        tracker.detect({}, 0.1, { hue: PRESETS[name], tolerance: 14, saturation: 85 }),
        null,
      );
      mat.delete();
    }
    ...
    const hands = [
      {
        ...
        landmarks: Array.from({ length: 21 }, () => ({ x: 70 / 320, y: 0.5 })),
      },
    ];
    const found = new BallTracker(cv).detect(
      {},
      0,
      { hue: 0, tolerance: 14, saturation: 85 },
      { hands },
    );
    assert.ok(Math.abs(found.x - 80) < 1);
...
test('rendered blur is accepted along established motion, but not as an initial ball', () => {
  ...
  try {
    ...
    for (const [i, x] of [60, 80, 100].entries()) {
      mat.setTo(new cv.Scalar(10, 10, 10, 255));
      cv.circle(mat, new cv.Point(x, 90), 10, new cv.Scalar(255, 0, 0, 255), -1);
      assert.ok(tracker.detect({}, i * 0.05, config));
    }
    mat.setTo(new cv.Scalar(10, 10, 10, 255));
    cv.ellipse(
      mat,
    ...
    const detected = tracker.detect({}, 0.15, config);
    assert.ok(detected);
    assert.equal(detected.blurred, true);
    assert.ok(Math.abs(detected.x - 120) < 1);
...
test('warm yellow close-up acquires by shape, but a warm elongated patch does not', () => {
  const mat = new cv.Mat(360, 640, cv.CV_8UC4, [25, 25, 25, 255]),
    original = cv.imread;
  ...
  try {
    ...
    assert.ok(ball);
    assert.ok(['shape', 'saturated', 'object'].includes(ball.mask_source));
    ...
    cv.rectangle(
      ...
      new cv.Point(400, 160),
      new cv.Scalar(201, 132, 58, 255),
      -1,
    );
    assert.equal(new BallTracker(cv).detect({}, 0, config), null);
  } finally {
...
test('yellow wall rectangles cannot acquire or sustain a ball track', () => {
  ...
  try {
    ...
    cv.rectangle(
      mat,
      new cv.Point(90, 60),
      new cv.Point(150, 120),
      new cv.Scalar(255, 220, 0, 255),
      -1,
    );
    assert.equal(tracker.detect({}, 0.1, config), null);
    assert.equal(new BallTracker(cv).detect({}, 0, config), null);
  } finally {
    cv.imread = original;
    mat.delete();
