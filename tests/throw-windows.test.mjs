...
test('26 seconds with three throws yields three padded full-motion windows', () => {
  ...
  for (const [i, w] of windows.entries()) {
    ...
    assert.ok(w.start_s <= start + 0.1, JSON.stringify(w));
  ...
  for (let i = 1; i < joined.frames.length; i++) {
    const before = joined.frames[i - 1],
      after = joined.frames[i];
    assert.ok(Math.abs(before.t_s + before.duration_s - after.t_s) < 0.000002);
  }
  assert.ok(
    Math.abs(joined.frames.at(-1).t_s + joined.frames.at(-1).duration_s - joined.duration_s) <
      0.000002,
  );
});
test('either throwing arm can produce pose-only candidates when ball is obscured', () => {
  for (const side of ['left', 'right']) {
    ...
    assert.equal(result[0].estimates[0].side, side);
...
test('idle motion, translations, predictions and an isolated landmark jump are not throws', () => {
  ...
  const translating = idle.map((s, i) => ({
    ...throwSample(i / 15, [], 'right', { shift: i * 0.02 }),
    ball_prediction: { observed: false, x_px: i * 30 },
  }));
...
test('brief tracking gaps do not create duplicate throws; clip edges are clamped', () => {
  const samples = throwSamples(6, [0, 4.5]);
  samples[8].pose = null;
  samples[13].ball = null;
...
test('opposite arm peaks and overlapping proposals collapse into one window', () => {
  const samples = throwSamples(5, [1]).map((s) => {
    ...
    for (const i of [11, 13, 15, 23]) s.pose[i] = left.pose[i];
    return s;
  });
...
test('edited windows sort chronologically and preserve source/output timestamps', () => {
  const result = buildTimeline(
    [
      { start_s: 20, end_s: 21.01, edited: true },
      { start_s: 3, end_s: 4.5 },
    ],
    26,
  );
  ...
  assert.deepEqual(
    result.segments.map((s) => [s.source_start_s, s.output_start_s, s.output_end_s]),
    [
      [3, 0, 1.5],
      [20, 1.5, 2.51],
    ],
  ...
  for (const windows of [
    ...
    assert.throws(() => validateWindows(windows, 26));
});
