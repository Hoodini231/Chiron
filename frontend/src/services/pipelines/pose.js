let vision;
const VISION_URL = new URL('/vendor/vision_bundle.mjs', window.location.origin).href;
const loadVision = () => (vision ??= import(/* @vite-ignore */ VISION_URL));
...
export function drawPose(ctx, landmarks, width, height) {
  if (!landmarks) return;
  const visible = (p) => p && (p.visibility ?? 0) >= 0.5 && (p.presence ?? 1) >= 0.5;
  ctx.strokeStyle = '#66c9ff';
  ctx.fillStyle = '#b1e4ff';
  ctx.lineWidth = Math.max(2, width / 500);
  for (const [a, b] of EDGES)
    if (visible(landmarks[a]) && visible(landmarks[b])) {
      ctx.beginPath();
  ...
  landmarks.forEach((p, i) => {
    if (i >= 11 && visible(p)) {
      ctx.beginPath();
      ctx.arc(p.x * width, p.y * height, Math.max(3, width / 320), 0, 2 * Math.PI);
      ctx.fill();
    }
  });
}
