...
function arm(sample, side, width, height) {
  ...
  if (scale < Math.hypot(width, height) * 0.04) return null;
...
export function detectThrowWindows(samples, duration, width, height) {
  ...
  for (const side of ['left', 'right']) {
    ...
    const bridged = measured.map((p, i) => {
      ...
      if (a < 0 || b >= measured.length || samples[b].t_s - samples[a].t_s > 0.21) return null;
      const mix = (samples[i].t_s - samples[a].t_s) / (samples[b].t_s - samples[a].t_s);
      return Object.fromEntries(
        ['x', 'y', 'angle'].map((k) => [
          k,
          measured[a][k] + mix * (measured[b][k] - measured[a][k]),
        ]),
      );
    });
    const points = bridged.map((p, i) => {
      ...
      return Object.fromEntries(
        ['x', 'y', 'angle'].map((k) => [
          k,
          [bridged[i - 1][k], p[k], bridged[i + 1][k]].sort((a, b) => a - b)[1],
        ]),
      );
    });
    const speeds = samples.map((s, i) => {
      const dt = i ? s.t_s - samples[i - 1].t_s : 0;
      return points[i] && points[i - 1] && dt > 0 && dt <= 0.21
        ? dist(points[i], points[i - 1]) / dt
        : null;
    });
    // Two supporting observations suppress single-frame landmark jumps.
    const smooth = speeds.map((v, i) =>
      v === null ? 0 : Math.min(v, Math.max(speeds[i - 1] ?? 0, speeds[i + 1] ?? 0)),
    );
    for (let i = 1; i < samples.length; i++) {
      if (smooth[i] < 2 || smooth[i] < (smooth[i - 1] ?? 0) || smooth[i] <= (smooth[i + 1] ?? 0))
        continue;
      const t = samples[i].t_s;
      const local = points.filter((p, j) => p && Math.abs(samples[j].t_s - t) <= 0.4);
      if (local.length < 3) continue;
      const angleRange =
        Math.max(...local.map((p) => p.angle)) - Math.min(...local.map((p) => p.angle));
      const excursion = Math.max(...local.map((p) => dist(p, points[i])));
      const nearby = samples.filter(
        (s) => Math.abs(s.t_s - t) <= 0.45 && s.ball?.observed !== false && s.ball,
      );
      const supported = nearby.some(
        (s, j) =>
          s.ball.tracking_phase === 'flight' &&
          nearby
            .slice(0, j)
            .some(
              (before) =>
                before.ball.tracking_phase === 'near_hand' &&
                before.ball.track_id === s.ball.track_id &&
                (!before.ball.hand_side || before.ball.hand_side === side),
            ),
      );
      if (!supported && (angleRange < 30 || excursion < 0.55)) continue;
      let start = i,
        end = i;
      while (start > 0 && t - samples[start - 1].t_s <= 2.5) {
        if (
          !points[start - 1] ||
          (smooth[start - 1] < 0.45 &&
            (smooth[start - 2] ?? 0) < 0.45 &&
            (smooth[start - 3] ?? 0) < 0.45)
        )
          break;
        start--;
      }
      while (end + 1 < samples.length && samples[end + 1].t_s - t <= 1.5) {
        if (
          !points[end + 1] ||
          (smooth[end + 1] < 0.45 && (smooth[end + 2] ?? 0) < 0.45 && (smooth[end + 3] ?? 0) < 0.45)
        )
          break;
        end++;
      }
      events.push({
        start_s: round(Math.max(0, samples[start].t_s - 0.25)),
        end_s: round(Math.min(duration, samples[end].t_s + 1 / SCAN_FPS + 0.25)),
        peak_s: t,
        side,
        confidence: supported ? 'ball_supported' : 'pose_only',
        strength: smooth[i],
      });
    }
  }
  ...
  for (const event of events.sort((a, b) => b.strength - a.strength)) {
    if (!peaks.some((p) => Math.abs(p.peak_s - event.peak_s) < 0.65)) peaks.push(event);
  }
  const windows = [];
...
export function validateWindows(windows, duration) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    const w = sorted[i];
    if (
      !Number.isFinite(w.start_s) ||
      !Number.isFinite(w.end_s) ||
      w.start_s < 0 ||
      w.end_s > duration ||
      w.end_s - w.start_s < 1 / ANALYSIS_FPS - 1e-6
    )
      throw new Error('Each window must be inside the video and at least one analysis frame long.');
    if (i && w.start_s < sorted[i - 1].end_s - 1e-6)
      throw new Error('Throw windows overlap. Adjust their start or end times.');
  }
  return sorted;
}
...
export function buildTimeline(windows, duration) {
  ...
  for (const [i, w] of validateWindows(windows, duration).entries()) {
    const length = w.end_s - w.start_s,
      start = output;
    const estimates = (w.estimates ?? []).map((e) => ({
      source_start_s: e.start_s,
      source_end_s: e.end_s,
      source_peak_s: e.peak_s,
      output_peak_s:
        e.peak_s >= w.start_s && e.peak_s < w.end_s ? round(start + e.peak_s - w.start_s) : null,
      side: e.side,
      confidence: e.confidence,
    }));
    const segment = {
      id: i + 1,
      source_start_s: w.start_s,
      source_end_s: w.end_s,
      output_start_s: round(start),
      output_end_s: round(start + length),
      source_start_analysis_frame: Math.floor(w.start_s * ANALYSIS_FPS),
      source_end_analysis_frame_exclusive: Math.ceil(w.end_s * ANALYSIS_FPS),
    ...
    for (let f = 0; f / ANALYSIS_FPS < length - 1e-8; f++) {
      ...
      frames.push({
        frame_index: index++,
        throw_id: i + 1,
        t_s: round(start + offset),
        source_media_time_s: round(w.start_s + offset),
        source_analysis_frame: Math.floor((w.start_s + offset) * ANALYSIS_FPS + 1e-6),
        duration_s: round(Math.min(1 / ANALYSIS_FPS, length - offset)),
      });
    }
    output += length;
  }
