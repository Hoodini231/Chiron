...
export class BallTracker {
  constructor(cv) {
    this.cv = cv;
    this.motion = new BallMotion();
  }
  reset() {
    this.motion.reset();
  }
  seed(x, y, time) {
    this.motion.seed(x, y, time);
  }
  get status() {
  ...
  detect(canvas, time, config, context = {}) {
    ...
    try {
      const src = own(cv.imread(canvas)),
        rgb = own(new cv.Mat()),
        hsv = own(new cv.Mat());
      ...
      const core = maskFor(
        Math.max(3, config.tolerance * 0.55),
        Math.min(255, config.saturation + 30),
      );
      const broad = maskFor(config.tolerance, config.saturation);
      const saturated = maskFor(
        Math.min(40, config.tolerance * 1.5),
        Math.min(240, config.saturation + 60),
      );
      ...
      for (const [maskSource, mask] of [
        ...
        for (let i = 0; i < contours.size(); i++) {
          ...
          try {
            ...
            if (area < 20 || area > src.rows * src.cols * 0.25 || !perimeter) continue;
            let circularity = Math.min(1, (4 * Math.PI * area) / (perimeter * perimeter));
            const rect = cv.boundingRect(contour),
              box = cv.minAreaRect(contour);
            const minor = Math.min(box.size.width, box.size.height),
              major = Math.max(box.size.width, box.size.height);
            if (minor < 2) continue;
            const aspect = major / minor;
            if (circularity < 0.12 || aspect > 7) continue;
            ...
            try {
              ...
              if (hullArea > 0 && area / hullArea >= 0.85 && hullPerimeter > 0) {
                circularity = Math.max(
                  circularity,
                  Math.min(1, (4 * Math.PI * hullArea) / (hullPerimeter * hullPerimeter)),
                );
                circleFill = Math.max(
                  circleFill,
                  hullArea / (Math.PI * circle.radius * circle.radius),
                );
              }
            } finally {
              hull.delete();
            }
            const angle = 0.5 * Math.atan2(2 * m.mu11, m.mu20 - m.mu02);
            let colourSum = 0,
              saturationSum = 0,
              count = 0;
            const stride = Math.max(1, Math.ceil(Math.sqrt((rect.width * rect.height) / 144)));
            for (let py = rect.y; py < rect.y + rect.height; py += stride)
              for (let px = rect.x; px < rect.x + rect.width; px += stride) {
                const pixel = py * src.cols + px;
                if (!mask.data[pixel]) continue;
                const difference = Math.abs(hsv.data[pixel * 3] - config.hue);
                colourSum +=
                  1 - Math.min(difference, 180 - difference) / Math.max(1, config.tolerance);
                saturationSum += hsv.data[pixel * 3 + 1] / 255;
                count++;
              }
            ...
            candidates.push({
              ...
              large: area > src.rows * src.cols * 0.1,
              strongShape:
                circularity >= 0.7 &&
                circleFill >= 0.72 &&
                aspect <= 1.5 &&
                count > 0 &&
                (saturationSum / count) * 255 >= Math.min(255, config.saturation + 30),
              sizeRadius: aspect > 1.8 ? minor / 2 : radius,
              colourScore: count
                ? Math.max(0, (0.7 * colourSum) / count + (0.3 * saturationSum) / count)
                : 0,
            });
          } finally {
            contour.delete();
      ...
      const gray = own(new cv.Mat()),
        contrast = own(new cv.Mat()),
        reduced = own(new cv.Mat()),
        circles = own(new cv.Mat());
      cv.cvtColor(src, gray, cv.COLOR_RGBA2GRAY);
      ...
      cv.resize(
        contrast,
        reduced,
        new cv.Size(320, Math.round(src.rows / scale)),
        0,
        0,
        cv.INTER_AREA,
      );
      ...
      cv.HoughCircles(
        ...
        16,
        3,
        Math.round((Math.min(src.rows, src.cols) * 0.3) / scale),
      );
      const matches = (x, y) =>
        x >= 0 && x < src.cols && y >= 0 && y < src.rows && shapeMask.data[y * src.cols + x] > 0;
      for (let i = 0; i < circles.data32F.length; i += 3) {
        const x = circles.data32F[i] * scale,
          y = circles.data32F[i + 1] * scale,
          radius = circles.data32F[i + 2] * scale;
        ...
        const segmented = candidates.some(
          (c) =>
            c.circularity >= 0.7 &&
            c.circleFill >= 0.72 &&
            c.aspect <= 1.8 &&
            Math.hypot(c.x - x, c.y - y) < radius * 0.4 &&
            Math.abs(c.radius - radius) < radius * 0.35,
        );
        ...
        for (let k = 0; k < 32; k++) {
          const a = (k * Math.PI) / 16,
            dx = Math.cos(a),
            dy = Math.sin(a);
          for (const ring of [0.2, 0.5, 0.8]) {
            const px = Math.round(x + dx * radius * ring),
              py = Math.round(y + dy * radius * ring);
            if (matches(px, py)) {
              inside++;
              saturation += hsv.data[(py * src.cols + px) * 3 + 1];
            }
          }
          if (matches(Math.round(x + dx * radius * 1.3), Math.round(y + dy * radius * 1.3)))
            outside++;
        }
        const fill = inside / 96,
          background = outside / 32;
        if (
          fill < 0.92 ||
          background > 0.65 ||
          saturation / Math.max(1, inside) < config.saturation + 15
        )
          continue;
        candidates.push({
          x,
