...
export default function CapturePage() {
  ...
  // Attach video/canvas once refs are ready
  useEffect(() => {
  ...
  const saveResult = async (report: any, processedBlob: Blob, rawBlob: Blob) => {
    ...
    try {
      const manifest = await api.saveResult(
        ...
        rawBlob.type || rawBlob.size ? rawBlob.type || 'video/webm' : 'video/webm',
      );
  ...
  return (
    <main className="layout capture-layout">
      <section className="video-pane">
        <video ref={videoRef} id="source" playsInline hidden={state.phase !== 'reviewing'} />
        <canvas
          ref={canvasRef}
          id="output"
        ...
        {!hasSource && (
          <div id="empty" style={{ position: 'absolute', textAlign: 'center', padding: 25 }}>
            ...
            <p style={{ fontSize: 16, color: '#9cacbe', marginBottom: 24 }}>
              Dodgeball throw analysis with body pose, hand tracking, and ball detection.
            </p>
            <div>
        ...
        <div className="video-controls">
          ...
          {state.errorMessage && (
            <p id="error" style={{ color: '#ffa9aa', fontSize: 13, margin: 0, flexBasis: '100%' }}>
              {state.errorMessage}
            </p>
          )}
