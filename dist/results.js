...
async function show(id) {
  ...
  const token=++loadToken; current=null; chatControls();$('chat-messages').replaceChildren();$('chat-input').value='';$('chat-status').textContent=''; $('generate-advice').disabled=true;
  try {
...
$('chat-form').onsubmit=async event=>{
  ...
  const id=current.id;chatBusy=true;chatControls();$('chat-status').textContent='Coach is replying…';
  try{
    const chat=await api(`/api/results/${id}/chat`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message})});
    drafts.delete(id);
    if(current?.id===id){current.chat=chat;renderChat();$('chat-input').value='';$('chat-status').textContent='Conversation saved.';}
  }catch(e){if(current?.id===id)$('chat-status').textContent=e.message;}
...
async function init(){
  try{
    ...
    $('llm-status').textContent=available?`Advice and chat send your messages, derived stats and sampled landmarks to ${config.provider}. Videos stay local.`:'LLM not connected. Configure GEMINI_API_KEY or OPENAI_API_KEY in .env and restart the local server.';
