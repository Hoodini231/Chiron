...
async function uploadVideo(file) {
  ...
  try {
    ...
    $('status').textContent =
      'Choose ball colour, then press Process video. Keep this page visible until it finishes.';
...
function frame(now, metadata) {
  ...
  if ((!seekProcessing && lastFrame === t) || video.readyState < 2) {
    scheduleFrame();
    return;
  }
  lastFrame = t;
...
async function startRecording() {
  if (!ready || !(stream || uploadedFile) || recording || finalizing || opening || pendingSave)
    return;
  if (uploadedFile) {
    opening = true;
    controls();
...
async function processUploadedVideo(r) {
  ...
  try {
    while (t < duration && recording === r && !r.failed) {
      ...
      $('status').textContent =
        `Processing frame ${r.samples.length} · ${t.toFixed(1)}s / ${duration.toFixed(1)}s`;
...
async function stopRecording() {
  ...
  if (!r.failed && r.chunks.length && r.samples.length) {
    ...
    if (r.file) {
      report.capture.original_filename = r.file.name;
      report.capture.original_duration_s = video.duration;
      report.capture.processed_segment_s = [0, duration];
    }
    ...
    $('playback').src = attach(
      'download-video',
      processedBlob,
      `throw-${stamp}-processed.${mime.includes('mp4') ? 'mp4' : 'webm'}`,
    );
    attach(
      'download-raw',
      rawBlob,
      `throw-${stamp}-original.${rawMime.includes('mp4') ? 'mp4' : 'webm'}`,
    );
    attach(
      'download-data',
    ...
    $('summary').textContent =
      `${duration.toFixed(1)}s · ${r.samples.length} frames · ball visible in ${Math.round((detected / r.samples.length) * 100)}%`;
    $('result').hidden = false;
