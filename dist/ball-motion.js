...
export function handAnchors(context, width, height) {
  ...
  for (const [index, hand] of (context.hands ?? []).entries()) {
    const points = (hand.landmarks ?? []).filter(valid).map(p => ({x:p.x * width, y:p.y * height}));
    if (points.length !== 21) continue;
    const palm = [0, 5, 9, 13, 17].map(i => points[i]);
    const centre = {x:palm.reduce((sum,p) => sum+p.x,0)/5, y:palm.reduce((sum,p) => sum+p.y,0)/5};
    const side = hand.associated_pose_side;
    anchors.push({...centre, points, radius:Math.max(4, ...palm.map(p => distance(p,centre))),
      id:side ? `pose:${side}` : `hand:${index}`, side:side ?? null, source:'hand_landmarks', reliability:1});
  }
  for (const [side, index] of [['left',15], ['right',16]]) {
    const wrist = context.pose?.[index];
    if (!valid(wrist) || (wrist.visibility ?? 0) < .6 || (wrist.presence ?? 1) < .5) continue;
    if (anchors.some(a => a.side === side)) continue;
    const point = {x:wrist.x * width, y:wrist.y * height};
    anchors.push({...point, points:[point], radius:6, id:`pose:${side}`, side,
      source:'pose_wrist', reliability:.55});
  }
  return anchors;
}
...
function proximity(candidate, anchor) {
  ...
  const reach = candidate.sizeRadius * 1.8 + anchor.radius * .5 + 8;
  return {anchor, gap, reach, closeness:clamp(1-gap/(reach*2)) * anchor.reliability};
}

export class BallMotion {
  constructor() { this.nextId=0; this.reset(); }
  reset() {
    this.last=null; this.velocity={x:0,y:0}; this.phase='searching';
    this.owner=null; this.separation=null; this.separationCount=0;
    this.status={state:'searching',phase:'searching',prediction:null,track_id:null};
    this.hint=null; this.lastTime=null;
  }
  seed(x,y,time) {
    this.reset();
  ...
  update(candidates,time,width,height,context={}) {
    ...
    if (this.last && time-this.last.t > .5) this.reset();
    this.lastTime=time;
    ...
    for (const candidate of candidates) {
      ...
      if (previous && (ratio<.3 || ratio>2.8)) continue;
      const near=anchors.map(a => proximity(c,a)).sort((a,b)=>b.closeness-a.closeness)[0];
      const colour=c.colourScore ?? 1;
      const shape=Math.min(clamp(circularity/.85),clamp((c.circleFill ?? 1)/.8));
      const size=previous ? Math.exp(-Math.abs(Math.log(ratio))) : clamp(Math.sqrt(c.area ?? Math.PI*c.radius*c.radius)/35);
    ...
    if (!best) {
      const canPredict=previous && dt<=.2 && predicted.x>=0 && predicted.x<width && predicted.y>=0 && predicted.y<height;
      this.status={state:canPredict?'predicted':previous?'lost':'searching',phase:this.phase,
        track_id:previous?.track_id ?? null,prediction:canPredict?{...predicted,radius:previous.radius,t:time,observed:false,age_s:dt}:null};
    ...
    if (!previous) {
      this.phase=currentNear && currentNear.gap<=currentNear.reach ? 'near_hand' : 'unassociated';
      ...
      if (reference) {
        ...
        if (reference.gap<=reference.reach) {
          this.phase='near_hand'; this.owner=reference.anchor;
          this.separationCount=0;
        } else if (owner && this.phase==='near_hand' && this.separation!==null && reference.gap>this.separation+Math.max(3,best.sizeRadius*.2)) {
    ...
    if (previous) {
      ...
      const weight=previous.observations===1 ? 1 : .7;
      this.velocity={x:weight*measured.x+(1-weight)*this.velocity.x,y:weight*measured.y+(1-weight)*this.velocity.y};
    }
