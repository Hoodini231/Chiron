...
export function checkCancelled(signal) {
  if(signal?.aborted) throw new DOMException('Processing cancelled. Your throw windows are kept.','AbortError');
}
...
export async function seekVideo(video, time, signal) {
  ...
  await new Promise((resolve,reject)=>{
    ...
    const fail=()=>{cleanup();reject(new Error('Could not read a video frame. Try again or choose another video.'));};
    const abort=()=>{cleanup();reject(new DOMException('Processing cancelled. Your throw windows are kept.','AbortError'));};
    const timer=setTimeout(fail,15000);
    video.addEventListener('seeked',done);video.addEventListener('error',fail);signal?.addEventListener('abort',abort,{once:true});
...
export async function scanThrows({video,analyze,reset,signal,progress}) {
  reset();
  const samples=[],duration=video.duration;
  for(let i=0;i/SCAN_FPS<duration;i++) {
    ...
    progress?.(t,duration);
    await yieldUI();checkCancelled(signal);
  }
  return detectThrowWindows(samples,duration,video.videoWidth,video.videoHeight);
}

export async function processThrows({video,windows,analyze,reset,createWriter,signal,progress}) {
  const timeline=buildTimeline(windows,video.duration),samples=[];
  checkCancelled(signal);
  ...
  try {
    ...
    for(const frame of timeline.frames) {
      checkCancelled(signal);
      if(frame.throw_id!==previous) {reset();previous=frame.throw_id;}
