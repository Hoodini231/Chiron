...
const unsupported = () => new Error('This browser cannot export trimmed video. Open the local app in a current Chrome or Edge browser.');
...
export async function createVideoWriter(canvas) {
  ...
  try {await output.start();}catch(e){await output.cancel();throw e;}
  return {
    add:(timestamp,duration)=>source.add(timestamp,duration),
    async finish(){
      await output.finalize();
      const blob=new Blob([target.buffer],{type:'video/webm'});
      if(blob.size>160*1024*1024) throw new Error('The processed video exceeds 160 MB. Shorten the throw windows and retry.');
      return blob;
    },
    async cancel(){if(output.state!=='finalized')await output.cancel();},
  };
}
