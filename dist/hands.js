...
export const HAND_NAMES=['wrist','thumb_cmc','thumb_mcp','thumb_ip','thumb_tip','index_mcp','index_pip','index_dip','index_tip','middle_mcp','middle_pip','middle_dip','middle_tip','ring_mcp','ring_pip','ring_dip','ring_tip','pinky_mcp','pinky_pip','pinky_dip','pinky_tip'];
...
export function drawHands(ctx,hands,width,height){
  ctx.strokeStyle='#ffbd70';ctx.fillStyle='#ffe1bd';ctx.lineWidth=Math.max(1.5,width/640);
  for(const hand of hands){const p=hand.landmarks;
    for(const [a,b]of EDGES){ctx.beginPath();ctx.moveTo(p[a].x*width,p[a].y*height);ctx.lineTo(p[b].x*width,p[b].y*height);ctx.stroke();}
    for(const l of p){ctx.beginPath();ctx.arc(l.x*width,l.y*height,Math.max(2,width/600),0,2*Math.PI);ctx.fill();}
  }
}
